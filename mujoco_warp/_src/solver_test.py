...
class SolverTest(parameterized.TestCase):
  ...
  def test_solve(self, cone, solver_, iterations, ls_iterations, sparse):
    ...
    for keyframe in range(3):
      ...
      mjw_cost = ctx.cost.numpy()[0] - ctx.gauss.numpy()[0]

      _assert_eq(mjw_cost, mj_cost, name="cost")
      ...
      mj_cost = cost(mjd.qacc)
      mjw_cost = cost(d.qacc.numpy()[0])
      self.assertLessEqual(mjw_cost, mj_cost * 1.025)
  ...
  def test_solve_batch(self, cone, solver_, iterations, ls_iterations):
    ...
    mjw_cost0 = cost(mjm0, mjd0, d.qacc.numpy()[0])
    self.assertLessEqual(mjw_cost0, mj_cost0 * 1.025)

    mj_cost1 = cost(mjm1, mjd1, mjd1.qacc)
    mjw_cost1 = cost(mjm1, mjd1, d.qacc.numpy()[1])
    self.assertLessEqual(mjw_cost1, mj_cost1 * 1.025)

    mj_cost2 = cost(mjm2, mjd2, mjd2.qacc)
    mjw_cost2 = cost(mjm2, mjd2, d.qacc.numpy()[2])
    self.assertLessEqual(mjw_cost2, mj_cost2 * 1.025)
